...
int i2c_write(uint32_t slave_addr, uint8_t *tx_buffer, uint32_t dlen) {
    BSC1_A = slave_addr;
    BSC1_DLEN = dlen;
...
int i2c_read(uint32_t slave_addr, uint8_t *rx_buffer, uint32_t dlen) {
    BSC1_A = slave_addr;
    BSC1_DLEN = dlen;
    ...
    // Start transfer with READ
    BSC1_C = C_I2CEN | C_ST | C_READ;

    uint32_t i = 0;
    while (i < dlen) {
        // Wait for data
        while (!(BSC1_S & S_RXD)) {
            if (BSC1_S & S_DONE) break;
        }
        if (BSC1_S & S_RXD) {
            rx_buffer[i++] = BSC1_FIFO & 0xFF;
        }
    }
